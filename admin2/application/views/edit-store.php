<div id="Container_Principale_full">
    <div id="Container_Principale_960">
        ...
        <form id="edit_store" method="post" action="<?=base_url()."storesmanager/update/"; ?>">
            <div>            
            ...
            <?$checked = ""; if ($store[0]->cart_active == '1') :?>
                <?$checked = "checked"?>
            <?endif?>
            <div class="Produit_liste_item"><label><?=gettext("Store id")?> : </label><input type='text' id='store_id' name='store_id' value="<?=$store[0]->store_id?>" /></div>
            <div class="Produit_liste_item"><label><?=gettext("Name")?> : </label><input type='text' id='name' name='name' value="<?=$store[0]->name?>"  /></div>
            <div class="Produit_liste_item"><label><?=gettext("Address")?> : </label><input type='text' id='address' name='address' value="<?=$store[0]->address?>" /></div>
            <div class="Produit_liste_item"><label><?=gettext("City")?> : </label><input type='text' id='city' name='city' value="<?=$store[0]->city?>" /></div>
            ...
            <div class="Produit_liste_item"><label><?=gettext("Manager")?> : </label><input type='text' id='manager_name' name='manager_name' value="<?=$store[0]->manager_or_owner?>" /></div>
            ...
            <input type="button" class="modalInput" rel="#supervisers" value="<?=gettext("Add store supervisers")?>" />
            <input type="button" class="commande" name="submit_update" value="<?=gettext("Update store infos")?>" onclick="add_update_store()"/>
            
            <div class="modal2" id="supervisers">
                <div id="Page_haut">
                    <div id="Breadcrumb"><a href="#">&nbsp;</a></div>
                    <h1><?= gettext("Store supervisers"); ?></h1>
                </div>                
                <div id="liste_overflow">                 
                    <?foreach($admins as $admin):?>
                        ...
                        <?if (count($admin_permissions)):?>
                            ...
                            <?endif?>
                        <?endif?>
                        <div class="Produit_liste_item"><input type='text' id='<?=$admin[0]->id?>' value="<?=$admin[0]->first_name?> <?=$admin[0]->family_name?>" readonly />
                        <input type="checkbox" value="<?=$admin[0]->id?>" id="active" name="user-permissions[]" <?=$checked?>/></div>            
                    <?endforeach?>
                <br/>
                </div>
                <input type="button" id="overlay_close" name="submit_update" value="<?=gettext("Save store supervisers")?>" onclick="close_overlay()"/>
            </div>
