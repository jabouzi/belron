...
function remove_users(users)
{
    ...
    {
        if ($("#checkbox_"+users[i]).attr("checked"))
        {
            users2remove.push(users[i]);
        }
    }

    $.get(root_url+"/usersmanager/delete/"+users2remove+'/',{uid : String((new Date()).getTime()).replace(/\D/gi, '') },
        function(data) {
            if (data != 0)
            {
                $(window.location).attr('href', root_url+'/usersmanager/index/'+$("#users_page").val()+'/'+$("#users_number").val()+'/'+$("#sort_page").val()+'/'+$("#sort_type").val());
            }
        }
    );
}
