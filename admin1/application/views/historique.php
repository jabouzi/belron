<div id="Container_Principale_full">
    <div id="Container_Principale_960">
        ...
        <select id="sort_number" onchange="change_history_number()">
        ...
        <div class="admin_pagination">
            <? if($page->prev()): ?>
                <a href="<?=base_url()."orders/historique/{$page->prev_page()}/{$number}"; ?>"><?=gettext("Previous")?></a>
            <? endif; ?>
            
            <? if($page->next()): ?>
                <a href="<?=base_url()."orders/historique/{$page->next_page()}/{$number}"; ?>"><?=gettext("Next")?></a>
            <? endif; ?>
        </div>
        ...
        <table width="100%" border="0" cellspacing="5" cellpadding="2" class="table_commande">
            ...
            <?foreach ($orders as $key => $order):?>
                ...
                <?if (count($order_detail)):?>                    
                    <? $class_approved = 'order_approuved'; $approved = gettext("approved").'(1/4)';?>
                    <? if (!$order->approved):?>
                ...
                <tr class="<?=$tr_class?>">
                    ...
                    <?if (count($order_detail)):?>                
                        <td><a href="<?=base_url()?>orders/detail/<?=$order->id?>"><?=gettext("Details")?></a></td>
                    <?endif?>                  
                </tr>
                <?endif?>
            <?endforeach?>
