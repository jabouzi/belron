<div id="Container_Principale_full">
    <div id="Container_Principale_960">
        ...
        <? else:?>
                ...
                <?$selected1 = "selected";?>
        <?endif?>
        <select id="sort_type" onchange="sort_orders_type()">            
        ...
        <div class="admin_pagination">
            <? if($page->prev()): ?>
                <a href="<?=base_url()."orders/lists/0/{$page->prev_page()}/{$sort}/{$type}/{$user_sup}/"; ?>"><?=gettext("Previous")?></a>
            <? endif; ?>
            
            <? if($page->next()): ?>
                <a href="<?=base_url()."orders/lists/0/{$page->next_page()}/{$sort}/{$type}/{$user_sup}/"; ?>"><?=gettext("Next")?></a>
            <? endif; ?>
        </div>
            ...
            <?foreach ($orders as $key => $order):?>
                ...
                <tr class="<?=$tr_class?>">
                    ...
                    <? if ((!$user_sup && !$has_superviser) || $user_sup):?>
                        <?if (!$order->approved && $order->pos != '' && !$supervised):?>
                            <td><a href="<?=base_url()?>orders/approve_order/<?=$order->id?>"><?=gettext("Approve")?></a></td>   
                        <?else:?>
                            <td></td>
                    <?endif?>
                    <?endif?>
                    <?if (count(unserialize($order->wish_list))):?>                
                        <td><a href="<?=base_url()?>orders/lists/<?=$order->id?>"><?=gettext("Details")?></a></td>
                    <?endif?>
                </tr>
                <?endif?>
            <?endforeach?>
