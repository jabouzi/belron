<div id="Container_Principale_full">
    <div id="Container_Principale_960">
        ...
        <select id="sort_number" onchange="change_dashboard_number()">
        ...
        <div class="admin_pagination">
            <? if($page->prev()): ?>
                <a href="<?=base_url()."admin/dashboard/{$page->prev_page()}/{$sort}/{$number}"; ?>"><?=gettext("Previous")?></a>
            <? endif; ?>
            
            <? if($page->next()): ?>
                <a href="<?=base_url()."admin/dashboard/{$page->next_page()}/{$sort}/{$number}"; ?>"><?=gettext("Next")?></a>
            <? endif; ?>
        </div>
        ...
        <table width="100%" border="0" cellspacing="5" cellpadding="2" class="table_commande">
            ...
            <?foreach ($orders as $key => $order):?>
                ...
                <tr class="<?=$tr_class?>">
                    ...
                    <?if (!$user_sup && (!$order->approved && !$supervised)):?>
                    ...
                    <?if (count(unserialize($order->wish_list))):?>                
                        <td><a href="<?=base_url()?>admin/order/<?=$order->id?>"><?=gettext("Details")?></a></td>
                    <?endif?>
                </tr>
                <?endif?>
            <?endforeach?>
